...
import contractModule from '../electron/updateContract.cjs';

const { updateMetadataFileName } = contractModule;
...
function requireTagForChannel(tag, channel) {
  const stable = /^v\d+\.\d+\.\d+$/.test(tag);
  const beta = /^v\d+\.\d+\.\d+-beta\.[1-9]\d*$/.test(tag);
  if ((channel === 'stable' && !stable) || (channel === 'beta' && !stable && !beta)) {
    throw new Error(`Release tag ${tag} is invalid for the ${channel} feed.`);
  }
}
...
export async function assembleUpdateMetadata({
  ...
  requireTagForChannel(tag, channel);
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository)) {
    throw new Error('Repository must use owner/name form.');
  }

  const source = yaml.load(await readFile(input, 'utf8'));
  if (!source || typeof source !== 'object' || Array.isArray(source)) {
    throw new Error('Update metadata must be a mapping.');
  }
  if (source.version !== tag.slice(1)) {
    throw new Error(`Update version ${source.version} does not match ${tag}.`);
  }
  if (!Array.isArray(source.files) || source.files.length === 0) {
    throw new Error('Update metadata contains no files.');
  }

  const seen = new Set();
  const releaseBase = `https://github.com/${repository}/releases/download/${encodeURIComponent(tag)}`;
  const files = [];
  const resolvedArtifactDirectory = path.resolve(artifactDir);
  for (const file of source.files) {
    ...
    seen.add(name);
    const target = path.resolve(resolvedArtifactDirectory, name);
    const relative = path.relative(resolvedArtifactDirectory, target);
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error(`Artifact escapes directory: ${name}`);
    }
    ...
    if (file.size !== undefined && file.size !== targetStat.size) {
      throw new Error(`Size mismatch for ${name}.`);
    }
    files.push({ ...file, size: targetStat.size, url: `${releaseBase}/${encodeURIComponent(name)}` });
  }
  ...
  if (source.path) {
    ...
    if (!seen.has(legacyName)) {
      throw new Error('Legacy update path does not match a files entry.');
    }
    const matchingFile = files.find((file) => artifactNameFromUrl(file.url) === legacyName);
    rewritten.path = `${releaseBase}/${encodeURIComponent(legacyName)}`;
    rewritten.sha512 = matchingFile.sha512;
  }
  const contents = `${yaml.dump(rewritten, {
    lineWidth: -1,
    noRefs: true,
    sortKeys: false
  }).trimEnd()}\n`;
  const metadataFileName = updateMetadataFileName(platform, arch, channel);
  const output = path.join(outputRoot, channel, platform, arch, metadataFileName);
  await mkdir(path.dirname(output), { recursive: true });
  await writeFile(output, contents, { flag: 'wx', mode: 0o644 });
  if (auditOutput) {
    await mkdir(path.dirname(auditOutput), { recursive: true });
    await writeFile(auditOutput, contents, { flag: 'wx', mode: 0o644 });
  }
  return { artifacts: [...seen], contents, output };
}
...
export { artifactNameFromUrl, requireTagForChannel };
