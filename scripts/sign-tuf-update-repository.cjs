...
const onlineRoles = Object.freeze(['targets', 'snapshot', 'timestamp']);
const expiryDays = Object.freeze({
  targets: 365,
  snapshot: 180,
  timestamp: 45
});
...
function publicKeyFromDescription(description) {
  if (
    ...
    throw new Error('Caul TUF root contains an unsupported public key.');
  }
...
function verifyEnvelope(envelope, root, roleName) {
  ...
  if (!role || !Number.isSafeInteger(role.threshold) || role.threshold < 1) {
    throw new Error(`Caul TUF root has no valid ${roleName} role.`);
  }
  ...
  if (valid.size < role.threshold) {
    throw new Error(`Caul TUF ${roleName} metadata does not meet its signature threshold.`);
  }
}
...
function requireRoot(root) {
  if (
    ...
    || !Number.isFinite(Date.parse(root.signed.expires))
    || Date.parse(root.signed.expires) <= Date.now()
  ) {
    throw new Error('Caul TUF root must be an unexpired reviewed version 1 document.');
  }
  verifyEnvelope(root, root, 'root');
  for (const roleName of ['root', ...onlineRoles]) {
    const role = root.signed.roles?.[roleName];
    if (role?.threshold !== 1 || role.keyids?.length !== 1) {
      throw new Error(`Caul TUF ${roleName} must use one distinct threshold-one key.`);
    }
  }
  const roleKeyIDs = ['root', ...onlineRoles].map((role) => root.signed.roles[role].keyids[0]);
  if (new Set(roleKeyIDs).size !== roleKeyIDs.length) {
    throw new Error('Caul TUF roles must not share keys.');
  }
}
...
function roleSigner(root, roleName, privateKeyPem) {
  ...
  if (
    ...
    throw new Error(`The supplied ${roleName} key does not match the reviewed Caul TUF root.`);
  }
...
function readPrevious(previousMetadataDirectory, roleName, root) {
  ...
  if (
    ...
    throw new Error(`Previous Caul TUF ${roleName} metadata is invalid.`);
  }
...
function signUpdateRepository({
  ...
  if (!Number.isFinite(now.getTime())) throw new Error('Caul TUF signing time is invalid.');
  if (
    !targetName
    || targetName === '.'
    || targetName === '..'
    ...
    throw new Error(`Unsafe Caul TUF target name: ${targetName}`);
  }
  if (fs.existsSync(outputDirectory)) {
    throw new Error('Caul TUF repository output must not already exist.');
  }
  const rootBytes = fs.readFileSync(rootPath);
  const root = JSON.parse(rootBytes);
  requireRoot(root);
  const targetBytes = fs.readFileSync(targetPath);
  const signers = Object.fromEntries(onlineRoles.map((roleName) => [
    roleName,
    roleSigner(root, roleName, privateKeys[roleName])
  ]));
  const previous = Object.fromEntries(onlineRoles.map((roleName) => [
    roleName,
    readPrevious(previousMetadataDirectory, roleName, root)
  ]));
  const versions = Object.fromEntries(onlineRoles.map((roleName) => [
    roleName,
  ...
  const targets = signedEnvelope({
    ...
    expires: expiresFrom(now, expiryDays.targets),
    targets: {
      [targetName]: {
  ...
  const snapshot = signedEnvelope({
    ...
    expires: expiresFrom(now, expiryDays.snapshot),
    meta: {
      'targets.json': {
        version: versions.targets,
  ...
  const timestamp = signedEnvelope({
    ...
    expires: expiresFrom(now, expiryDays.timestamp),
    meta: {
      'snapshot.json': {
        version: versions.snapshot,
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  const privateKeys = Object.fromEntries(onlineRoles.map((roleName) => {
    const variable = `CAUL_TUF_${roleName.toUpperCase()}_PRIVATE_KEY_PEM`;
    if (!env[variable]) throw new Error(`Missing ${variable}.`);
    return [roleName, env[variable]];
  }));
  const configuredNow = env.CAUL_TUF_METADATA_NOW
    ? new Date(env.CAUL_TUF_METADATA_NOW)
    : new Date();
  ...
  process.stdout.write(
    `Signed Caul TUF ${result.targetName} at versions `
    + `${onlineRoles.map((role) => `${role}=${result.versions[role]}`).join(', ')}.\n`
  );
}
