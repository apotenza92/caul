...
- Seamless local AI responses use Caul-owned local runtimes under app `userData`: `llama.cpp` with GGUF models on supported platforms, and MLX LM with MLX models on Apple Silicon. Caul starts the selected runtime as a loopback-only child process on demand and routes Local AI requests through it. Local AI defaults to compact models for fast live-call answers, even on larger machines. The local path does not require or probe external model runtimes.
