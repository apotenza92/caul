...
- Bundle Pi only after its package licence has been verified locally during packaging. Pi must run with Susura-owned configuration under the app `userData` directory and must never read or modify global `~/.pi/agent` state. ChatGPT sign-in should open in the user's default browser and return through Pi's local callback server, rather than using a separate Electron auth window.
