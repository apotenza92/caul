...
Release validation uses Parallels VM E2E gates for macOS, Windows and Ubuntu Linux. The gates are split into explicit prepare, package, stage and E2E phases so the packaged app under test is version-matched before GUI automation starts. The gates must prove packaged launch or install, onboarding, audio capture, automated input/output source isolation, local transcription, renderer AI response and pre-setup privacy. Normal macOS release E2E proves native capture first, then uses smoke-ready setup state for onboarding completion; fresh macOS permission prompt testing is a separate explicit permission-flow check. macOS and Windows require Electron content protection on both the overlay and floating handle. Linux records the same protection path as best effort because Electron content protection is only supported on macOS and Windows. Fedora currently remains an RPM install/package gate.
