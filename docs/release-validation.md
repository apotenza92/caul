...
The `CI` workflow is manually dispatchable and callable by the tag-only release workflow. It runs actionlint from a checksum-pinned upstream archive, the renderer and TypeScript checks, the complete deterministic Rust test suite, bundled dependency licence verification and native build preflights on macOS ARM64, Windows x64 and Linux x64. The macOS preflight selects and verifies Xcode 26.4.1 explicitly instead of accepting the runner's mutable default. Run it deliberately against a release-preparation branch before merging; the release workflow reruns the same gate against the exact tag.

For dependency changes, run `npm run audit:runtime` after a clean install. It audits the installed production tree after Caul's reviewed Pi preparation step has removed the vulnerable copies declared by Pi's upstream shrinkwrap and redirected Pi to the exact maintained root runtimes. A raw lockfile audit may still report those upstream shrinkwrap entries even though they are absent from the tested and packaged runtime.

The release workflow fails before building unless GitHub reports that the repository is public, the default branch is `main`, and the exact release tag commit is reachable from `main`. It calls the same `CI` workflow before any release packaging begins, then builds the complete supported package matrix on GitHub-hosted macOS, Windows and Linux runners. A tag must not publish a release when the shared checks or any platform build fails.
...
Linux release verification extracts AppImage, Debian and RPM packages on each supported native architecture. AppImages use Electron Builder's checksum-pinned static runtime. Verification checks every packaged ELF for architecture and GLIBC compatibility, then checks `ldd` closure from the actual application, helper and native-module roots under the same library path used by the AppImage launcher. Toolset compatibility shims are not treated as independent entry points, but any shim reached by the application must still resolve completely. The gate rejects unresolved runtime dependencies, a required GLIBC symbol newer than `GLIBC_2.39`, missing application metadata, or incomplete desktop-entry and icon integration. This is the declared Ubuntu 24.04-class runtime ceiling, not a claim of compatibility with older distributions. The x64 RPM additionally undergoes a package-manager-native N-1 upgrade in a digest-pinned Fedora 42 container: `dnf` resolves the package dependency closure, stable and beta launch, user data survives the upgrade, desktop integration remains registered, and uninstall removes the applications without deleting user data. Caul RPMs suppress rpmbuild's global build-ID symlinks because stable and beta intentionally contain identical native binaries under separate product paths. The Fedora gate migrates historical packages one channel at a time, then requires both corrected candidates to coexist.
...
Release macOS jobs use Xcode 26.4.1, a Developer ID Application certificate exported as PKCS #12 and a separate App Store Connect API key for `notarytool`. Apple ID passwords and app-specific-password notarisation are not supported release paths.
...
Release candidates also run the macOS updater from a previous public package for each applicable channel. The valid-update gate runs once from the latest eligible GitHub release archive and once from the currently published Homebrew cask, which may lag the GitHub release after a fail-closed cask publication. Both routes exercise Caul's own signed in-app updater against the same candidate, without invoking Homebrew for the update. Homebrew remains an independent upgrade route through `brew upgrade --cask`; installing by Homebrew must not disable or replace the in-app updater. The gate also proves rejection of a corrupt archive and an ad-hoc signed archive. The updater jobs use the tag-restricted `stable-updater-verification` and `beta-updater-verification` environments, which have no reviewer and contain no signing credentials. A channel with no prior public package may skip N-1 only when the exact candidate tag is named by `MACOS_UPDATER_BOOTSTRAP_TAG` in that channel's updater-verification environment. Remove that variable after the bootstrap release and never advance it to bypass later N-1 tests.
...
Before creating any release tag, run the applicable hardware checks from the exact commit that will be tagged. After they pass, set the repository variable `CAUL_MACOS_HARDWARE_EVIDENCE_COMMIT` to that full 40-character commit SHA. The tag-only workflow rejects every other commit before deterministic CI or package publication. The variable records the scope of approved evidence, not secret material, and must never be advanced without rerunning the real hardware checks.
...
Caul checks for app updates weekly by default, with other check frequencies available in Settings. Checks do not silently download or install software. A user must start the download and explicitly restart to install on macOS, Windows NSIS and Linux AppImage. Debian and RPM users manually download the verified package and install it with their distribution package manager.
