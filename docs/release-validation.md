...
The one-time Windows ARM64 migration from `0.1.21` is a documented legacy recovery because those authenticated public installers exit successfully but cannot restore their application executable. The candidate installer recognises only that exact registered version with its expected executable absent, clears the stale installation and uninstall registration, and overwrites the partial files in place without deleting user data or an arbitrary installation directory. For this migration only, set `WINDOWS_ARM64_LEGACY_PUBLIC_BOOTSTRAP_TAG` to the exact candidate tag. The gate must reproduce the partial stable and beta installations, install the candidate over each partial state, preserve seeded user data, verify and launch both candidate applications, and prove coexistence. x64 still performs the full prior launch and N-1 transition. Remove the variable immediately after the successful public release and never advance it.
